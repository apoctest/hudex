...
export default async function handler(
    ...
    try {
        ...
        const poolDataCheck = includePage("dex%2Cdex.network%2Cpool_metric%2Ctokens", 1)
        const poolCheck = includeNetMet(poolDataCheck)
        const apiBase = buildUrl(slug) + "/" + "/pools"
        ...
        const poolData = includePage("dex%2Cdex.network%2Cpool_metric%2Ctokens", Number(currentPage))
        ...
        const networkInfo = poolsInfo["included"].filter((net: { type: string }) => net.type === "network")
        ...
        for (let i = 0; i < poolsInfo["data"].length; i++) {
            ...
            const dexGeneral = dexInfo.filter((tkn: { id: any }) => tkn.id === thePair["relationships"]["dex"]["data"].id.toString())[0]
            const dex = dexGeneral["attributes"]
            const dexNetworkID = dexGeneral["relationships"]["network"]["data"].id

            const netv = networks["networks"].filter(net => net.attributes.identifier === dexNetworkID)
            const attr = netv["attributes"]
