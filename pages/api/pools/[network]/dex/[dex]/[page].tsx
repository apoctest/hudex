...
export default async function handler(
    ...
    const { network, page, dex } = req.query
    ...
    try {
        ...
        const poolDataCheck = includePage(dex + "&include=dex%2Cdex.dex_metric%2Cdex.network%2Ctokens", 1)
        ...
        const poolsCheck = await fetch(apiBase + "?dex=" + poolCheck)
        ...
        const poolData = includePage(dex + "&include=dex%2Cdex.dex_metric%2Cdex.network%2Ctokens", Number(currentPage))
        ...
        const pools = await fetch(apiBase + "?dex=" + poolInclude)
        ...
        const dexMetric = poolsInfo["included"].filter((net: { type: string }) => net.type === "dex_metric")    
        const dexAttr = dexInfo[0]["attributes"]
        ...
        res.status(200).json({
            ...
            dexInfo: {
                name: dexAttr.name,
                img: dexAttr.image_url,
                identifier: dexAttr.identifier,
                txns24H: dexMetric[0]["attributes"].swap_count_24h,
                volume24H: dexMetric[0]["attributes"].swap_volume_usd_24h,
                pairs: pairList
            }
        })
    } catch (error) {
